    ...
    private: 
        ...
        std::vector<geometry_msgs::msg::Point> find_path_on_level_heuristic(geometry_msgs::msg::Point start,geometry_msgs::msg::Point goal ){
            ...
            while(!pq.empty()){
                ...
                if (visited[current->point.x][current->point.y][current->point.z]){
                    continue;
                }
                visited[current->point.x][current->point.y][current->point.z] = true;
